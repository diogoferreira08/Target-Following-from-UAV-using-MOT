...
with rosbag.Bag(rosbag_path, 'r') as bag:
    for topic, msg, t in bag.read_messages(topics=[topic_name]):
        ...
        error_center.append(np.sqrt((msg.distance_cnst - 640.5)**2 + (msg.distance_singer - 360.5)**2))
...
plt.figure(1)
plt.plot(error_heading_values, label='heading Error')
plt.legend()

#print(error_heading_values)
...
axes[0, 1].plot(timestamps, real_distances1, label='Real Distance 1')
...
error_y = np.subtract(real_target_y,estimated_target_y)
